...
# persistent model serialization (joblib is recommended for sklearn objects)
try:
...
def _get_host(url: str) -> str:
    """
    Extract host from a URL string robustly using urllib.parse.
    Accepts strings lacking a scheme by prepending 'http://' for parsing.
    Returns lower-cased hostname (without port).
    """
    if "://" not in url:
    ...
    host = parsed.hostname or url  # fallback to original if parsing fails
    return host.lower()


def _is_ip(host: str) -> int:
    """
    Returns 1 if host is a valid IPv4 or IPv6 address, otherwise 0.
    Uses ipaddress for proper validation (handles ranges correctly).
    """
    try:
...
def extract_features(url: str) -> List[float]:
    ...
    Features:
      ...
      - digit ratio in host
      - host entropy
      - suspicious token count (from SUSPICIOUS_TOKENS)
    """
    host = _get_host(url)
    features: List[float] = [
        ...
        url.count("@"),                 # number of '@'
        int(url.lower().startswith("https")),   # has https
        int(bool(__import__("re").search(r"\d", url))),  # contains any digit
...
def train_model(
    ...
    # Save model using joblib if available, else fallback to pickle (with warning)
    try:
        if joblib_dump is None:
            # fallback to pickle if joblib isn't installed
            import pickle  # type: ignore
            with model_path.open("wb") as f:
                pickle.dump(clf, f)
            logger.warning("joblib not available; saved model with pickle (still works, but joblib is recommended).")
        else:
...
def load_model(model_path: Path = MODEL_PATH) -> Optional[RandomForestClassifier]:
    """
    Load a joblib/pickle model. Returns None if model cannot be loaded or doesn't exist.
    Performs some basic validation (has predict_proba and classes_).
    """
    ...
    try:
        if joblib_load is None:
            ...
            with model_path.open("rb") as f:
                model = pickle.load(f)
            logger.warning("joblib not available; loaded model with pickle.")
        else:
            model = joblib_load(model_path)

        # basic validation
        if not hasattr(model, "predict_proba") or not hasattr(model, "classes_"):
            logger.error("Loaded object is not a valid sklearn classifier (missing predict_proba/classes_).")
            return None
...
# FastAPI app
# ---------------------------
app = FastAPI(title="Phishing URL Checker (improved single-file)")
...
def check_url(req: URLRequest):
    ...
    Returns percentages (0-100) rounded to 2 decimals.
    """
    model: Optional[RandomForestClassifier] = getattr(app.state, "model", None)
    if model is None:
        raise HTTPException(status_code=503, detail="Model not available. Train model first (python KI.v2.py train).")
    ...
    # if 0/1 classes are present, use them; otherwise fall back to first/last indices
...
def serve(host: str = "127.0.0.1", port: int = 8000, reload: bool = False) -> None:
    ...
    Starts uvicorn with the app object directly which avoids module import name issues.
    """
    if uvicorn is None:
    ...
    # Use app object directly (works whether running as script or module)
...
def quick_predict(url: str) -> Optional[Dict[str, float]]:
    ...
    Returns dict with percentage floats.
    """
    model = load_model(MODEL_PATH)
    if model is None:
        logger.error("Model not found. Train first using 'python KI.v2.py train'")
        return None
    features = [extract_features(url)]
    probs = model.predict_proba(features)[0]
    class_to_index = {c: i for i, c in enumerate(model.classes_)}
...
def main(argv: Optional[List[str]] = None) -> None:
    parser = argparse.ArgumentParser(description="Phishing URL checker (train + serve + test) — improved single file")
    sub = parser.add_subparsers(dest="cmd", required=True)

    p_train = sub.add_parser("train", help="Train a model and save to models/model.joblib")
    ...
    p_predict = sub.add_parser("predict", help="Quick local predict (loads models/model.joblib)")
